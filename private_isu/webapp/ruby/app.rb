  ...
  class App < Sinatra::Base
    ...
    helpers do
      ...
      def make_posts(results)
        ...
        user_ids = posts.to_a.map{|p|p[:user_id]}
        user_ids += comments.to_a.map{|p|p[:user_id]}
        users = db.prepare(user_query).execute(user_ids.map(&:to_i))
        puts "user_count: #{users.count}"
        results.to_a.each do |post|
    ...
    get '/' do
      ...
      results = db.query('SELECT posts.id as id, posts.user_id as user_id, posts.body as body, posts.created_at as created_at, posts.mime as mime FROM `posts` JOIN `users` ON posts.user_id = users.id WHERE users.del_flg = 0 ORDER BY posts.created_at DESC LIMIT 20')
